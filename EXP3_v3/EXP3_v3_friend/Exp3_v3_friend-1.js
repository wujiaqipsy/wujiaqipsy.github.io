            ...
            stimuli: [
                {
                    ...
                    show_start_time: 500,
                    show_end_time: 1100// 1100ms after the start of the trial
                },
                {
                    obj_type: "image",
                    file: function () { return jsPsych.timelineVariable("Image")() },
                    startX: "center", // location of the cross's center in the canvas
                    startY: -200,//-250目前参数肉眼等距
...
// 知觉匹配任务：正式实验部分（正式实验过程鼠标消失，实验结束后鼠标出现）
let matching_task = {
    timeline: [
        {
            type: jsPsychPsychophysics,
...
var feedback_p = {
    ...
    stimulus: function () {
        ...
        return "<style>.context{color:white; font-size: 35px; line-height:40px}</style>\
                              <div><p class='context'>您正确回答了" + accuracy + "% 的试次。</p>" +
            "<p class='context'>您的平均反应时为" + rt + "毫秒。</p>" +
            "<p class='context'>请按任意键继续。</p>";
    }
}

var instr_repractice_friend = { 
    type: jsPsychInstructions,
    pages: function () {
